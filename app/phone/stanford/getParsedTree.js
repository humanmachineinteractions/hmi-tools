var ParsedTree = (function() {
  ...
  ParsedTree.prototype.parseRecursive = function(treeString, currentPosition) {
    ...
    if (!isBracket) {
      var t = splittedTokens[1];
      // all for rebuilding properly... todo extract refactor, etc
      if (!ignore(t)) {
        if (needs_no_space_before(t))
...
var getParsedTree = function(treeString) {
  return new ParsedTree(treeString);
};

module.exports = getParsedTree;

//?!:
var nnnn = ['.', ',', "'", '"', '?', '!', ':', '%', "'d", "'m", "'n", "'s", "'t", "n't", "'re"];
function needs_no_space_before(t){
  return nnnn.indexOf(t) != -1;
}

var iiiii = ['``', "''", "-LRB-", "-LRB-", "-RRB-"]
function ignore(t) {
  return iiiii.indexOf(t) != -1;
}
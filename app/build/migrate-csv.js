...
var converter = require('./gutil');
var sox = require('sox');
var utils = require('../utils');


var r = path.join(__dirname, 'data', 'test-small-src');
var dest = path.join(__dirname, 'data', 'test-small-1');
...
function process_line(dir, line, name, text, complete) {
  ...
  //converter.convert(wav, dwav1, 'wav', null, 16000, function (err, info) {
  //  console.log(info)
  //  fs.writeFile(dtxt, text, function (err) {
  //    console.log("!", cc, err, name, text);
  //    cc++;
...
function soxConvert(source, target, cb) {
  ...
  // line 126 ->       '-b', '16',
  var job = sox.transcode(source, target, {channelCount: 1, sampleRate: '16k', bits: /* see modified module */ '16', format: 'wav'});
